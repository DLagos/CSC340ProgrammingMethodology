...
Matrix<T> Matrix<T>::operator*(const Matrix<T> & aMatrix) {
    ...
    Matrix<T> newMatrix(rows, aMatrix.cols);
    for (int row = 0; row < rows; row++) {
        for (int col = 0; col < aMatrix.cols; col++) {
            for (int inner = 0; inner < aMatrix.rows; inner++) {
                newMatrix.matrix[row][col] += matrix[row][inner] * aMatrix.matrix[inner][col];
            }
...
Matrix<T> Matrix<T>::operator+(const Matrix<T> & aMatrix){
    ...
    for (int row = 0; row < rows; row++) {
        for (int col = 0; col < cols; col++) {
            newMatrix.matrix[row][col] = matrix[row][col] + aMatrix.matrix[row][col];
        }
    }
    return newMatrix;
}
...
Matrix<T> Matrix<T>::operator*(const T & scalar){
    ...
    for (int row = 0; row < rows; row++) {
        for (int col = 0; col < cols; col++) {
            newMatrix.matrix[row][col] = matrix[row][col] * scalar;
        }
    }
    return newMatrix;
}